...
namespace MetodosPontoFrequencia.HorasExtras
{
    public class HorasExtrasDAO
    {
        public void Incluir(string IDVinculoUsuario, string CodVerba, string data, string QtdeHoras, string idUsuarioInclusao, string idEmpresa)
        {
            ...
            sbScalar.AppendLine("END");
            sbScalar.AppendLine("IF((@HorasMes >= 40.1 AND " + CodVerba + " = 1225) OR (@HorasMes > 129 AND " + CodVerba + " = 1244))");
            sbScalar.AppendLine("BEGIN");
            sbScalar.AppendLine("	RAISERROR ('O LIMITE MÁXIMO DE HORAS DO MÊS JÁ FOI PREENCHIDO', 16, 1 );");
            sbScalar.AppendLine("END");
            sbScalar.AppendLine("IF(@HorasDia >= 2.1 AND " + CodVerba + " = 1255)");
            sbScalar.AppendLine("BEGIN");
            ...
            //sb.AppendLine("BEGIN");
            //sb.AppendLine("SET @TipoVerba = (CASE WHEN " + CodVerba + " = 1255 THEN 'HORAS EXTRAS' ELSE 'ADICIONAL NOTURNO' END)");
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("SELECT Sequencial Codigo,CodigoVerba, CASE WHEN CodigoVerba = 1244 THEN 'Adicional Noturno' WHEN CodigoVerba = 1255 THEN 'Hora Extra' END Verba, DataOcorrencia, TotalHoras");
        ...
        public List<HorasDiaModel> GetDadosWS(string IDEmpresaOrgao_eTurmalina, string MesRef, string AnoRef)
        {
            ...
            sb.AppendLine("SELECT DISTINCT VU.Matricula,  U.Login CPF, CONVERT(VARCHAR(5),HL.TotalHoras, 108) Hora, HL.CodigoVerba,  CONVERT(VARCHAR(12),HL.DataOcorrencia,103) DataOcorrencia, '' Situacao");
            sb.AppendLine("FROM dbo.TBHorasExtrasLancamentos HL");
            sb.AppendLine("JOIN dbo.TBVinculoUsuario VU ON (VU.IdVinculoUsuario = HL.IdVinculoUsuario)");
            sb.AppendLine("JOIN dbo.TBUsuario U ON (U.IdUsuario = VU.IdUsuario)");
            sb.AppendLine("JOIN dbo.TBEmpresa E ON (VU.IdEmpresa = VU.IDEmpresa)");
